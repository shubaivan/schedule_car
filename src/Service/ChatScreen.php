 ...
 *
 * Тепер натискання на застаріле повідомлення нікого не плодить: у старого
 * знімається клавіатура, а живим лишається рівно один екран.
 *
 * Правило, де саме він малюється:
 *  - натиснули кнопку САМОГО екрана — правимо його на місці (людина дивиться
 *    на це повідомлення, воно й має змінитись);
 *  - усе інше (команда, текст у формі, кнопка старої картки) — екран переїжджає
 *    вниз чату новим повідомленням, а попередній видаляється. Інакше відповідь
 *    тихо переписує повідомлення десь угорі над репліками людини, і бот
 *    виглядає мертвим — саме так 18.08 «мовчали» /start і /postachannia.
 */
class ChatScreen
{
    ...
    public function render(Nutgram $bot, string $text, ?InlineKeyboardMarkup $markup = null): ?int
    {
        ...
        // Перемальовуємо на місці ЛИШЕ те повідомлення, кнопку якого щойно
        // натиснули, і лише якщо воно і є поточний екран. Усе інше — команда,
        // текст у формі, кнопка старої картки — малює новий екран унизу чату.
        //
        // 18.08: правка на місці «мовчала». Людина слала /start, /postachannia,
        // /crm — бот справно їх приймав, але переписував повідомлення, яке вже
        // поїхало вгору над її власними; унизу чату не з'являлось нічого, і
        // виглядало це як мертвий бот.
        ...
        // Попередній екран прибираємо — інакше в чаті лишиться два робочих.
        $this->close($bot);
    ...
    public function close(Nutgram $bot): void
    {
        ...
        if ($chatId !== null && $current !== null) {
            try {
                ...
                // Старіше за 48 годин Telegram видалити не дасть — тоді хоча б
                // знімаємо кнопки, щоб екран-привид більше нікого не покликав.
                $this->disarm($bot, $chatId, $current);
            }
        }
